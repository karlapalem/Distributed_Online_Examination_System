...
/**
 * Servlet implementation class InstructerRemoveExampaperpage
 */
@WebServlet("/InstructerRemoveExampaperpage")
public class InstructerRemoveExampaperpage extends HttpServlet implements Serializable{
	private static final long serialVersionUID = 1L;
       
    
    public InstructerRemoveExampaperpage() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
	 	...
	 	ResultSet rs=d.ExampapersAvailDowloadDiplay(maintopic,subtopicname,type,name);
	 ...
	 //	out.println("      <h11><marquee> <p >Software and Web Applications Security:State-of-the-Art Courseware and Learning Paradigm</p></marquee></h11>");
	 	...
	 	int i=1;
   		out.println("<form action='http://www.examination.com/PASS/InstructerRemoveExampaper'>" 
		+"<input type='hidden' name='maintopic' id= 'maintopic' value='"+request.getParameter("maintopic")+"'/>"
		+"<input type='hidden' name='subtopicname' value='"+request.getParameter("subtopicname")+"'/>"
   		+"<input type='hidden' name='name' value='"+name+"'/>" );	
   		
	 	out.println("    <h3> Click on check Box which tests wants to delete</h3>");
	 	out.println("     <h4> <ul class=\"categories\">");
	 	if(rs==null)
	 	{out.println("Exam Papers are not available<br><br>");}
	 	else{
	 	while(rs.next())
		{
